...
// The essential include for the reflection library
...
namespace ShaderReflector {
    ...
    ReflectedLayout reflect(const std::map<RAL::ShaderStage, const std::vector<char>*>& shader_stages) {
        ...
        for (const auto& [stage, bytecode] : shader_stages) {
            ...
            // --- Reflect Descriptor Sets (UBOs, Samplers, etc.) ---
            auto process_resources = [&](const spirv_cross::SmallVector<spirv_cross::Resource>& resource_list) {
                for (const auto& res : resource_list) {
                    ...
                    // Check if this binding already exists (e.g., from another stage)
                    ...
                    if (it != layout_desc.bindings.end()) {
                        // Binding exists, just merge the shader stage flag
                        it->stages |= stage;
                    } else {
                        // New binding, add it to the layout description
                        RAL::DescriptorSetLayoutBinding new_binding;
                        new_binding.binding = binding;
                        new_binding.stages = stage;
            ...
            process_resources(resources.sampled_images);
            // ... process other resource types like storage_buffers here ...
        ...
        if (max_push_constant_size > 0) {
            layout.pushConstantRanges.push_back({
                accumulated_push_constant_stages,
                0, // Offset is almost always 0
