...
namespace RDE {
    ...
    void TestSceneLayer::create_triangle_resources() {
        if (!m_device) {
            RDE_CORE_WARN("TestSceneLayer: no device - cannot create triangle resources");
            return;
        }
        ...
        vbDesc.usage = RAL::BufferUsage::VertexBuffer;
        vbDesc.memoryUsage = RAL::MemoryUsage::HostVisibleCoherent;
        ...
        if (!assetPath) {
            RDE_CORE_ERROR("Asset path unavailable - triangle shaders missing");
            return;
        }
        ...
        if (vertCode.empty() || fragCode.empty()) {
            RDE_CORE_ERROR("Failed to read triangle SPIR-V binaries");
            return;
        }
        ...
        RAL::GraphicsShaderStages stages{};
        stages.vertexShader = m_triangleVS;
        stages.fragmentShader = m_triangleFS;
        pDesc.stages = stages;
        ...
        RAL::PushConstantRange pc{};
        pc.stages = RAL::ShaderStage::Vertex;
        pc.offset = 0;
        pc.size = sizeof(glm::mat4);
        pc.name = "Model";
        pDesc.pushConstantRanges.push_back(pc);
        // Vertex layout
        RAL::VertexInputBinding binding{};
        binding.binding = 0;
        binding.stride = sizeof(Vertex);
        pDesc.vertexBindings.push_back(binding);
        RAL::VertexInputAttribute attrPos{};
        attrPos.location = 0;
        attrPos.binding = 0;
        attrPos.format = RAL::Format::R32G32B32_SFLOAT;
        attrPos.offset = 0;
        attrPos.name = "POSITION";
        pDesc.vertexAttributes.push_back(attrPos);
        RAL::VertexInputAttribute attrColor{};
        attrColor.location = 1;
        attrColor.binding = 0;
        attrColor.format = RAL::Format::R32G32B32_SFLOAT;
        attrColor.offset = sizeof(float) * 3;
        attrColor.name = "COLOR";
        pDesc.vertexAttributes.push_back(attrColor);
    ...
    void TestSceneLayer::destroy_triangle_resources() {
        if (!m_device) return;
        if (m_trianglePipeline.is_valid()) {
            m_device->destroy_pipeline(m_trianglePipeline);
            m_trianglePipeline = RAL::PipelineHandle::INVALID();
        }
        if (m_triangleVS.is_valid()) {
            m_device->destroy_shader(m_triangleVS);
            m_triangleVS = RAL::ShaderHandle::INVALID();
        }
        if (m_triangleFS.is_valid()) {
            m_device->destroy_shader(m_triangleFS);
            m_triangleFS = RAL::ShaderHandle::INVALID();
        }
        if (m_triangleVertexBuffer.is_valid()) {
            m_device->destroy_buffer(m_triangleVertexBuffer);
            m_triangleVertexBuffer = RAL::BufferHandle::INVALID();
        }
    }
}
