...
namespace RDE {
    ...
    void EditorLayer::on_event(Event &e) {
        ...
        dispatcher.dispatch<MouseButtonPressedEvent>([this](MouseButtonPressedEvent &ev) {
            ...
            if (pick_at_cursor(hit)) {
                m_selection_context.last_selected_entity = hit;
                return true;
            }
            return false;
        });
    ...
    void EditorLayer::on_render_gui() {
        ...
        if (ImGui::BeginMenu("Editor")) {
            if (ImGui::MenuItem("Create Empty")) {
                auto e = m_scene.get_registry().create();
                Require<TransformLocal>(registry, e);
                Require(registry, e, NameTagComponent{"Entity"});
                m_selection_context.last_selected_entity = e;
            }
            bool hasSel = m_selection_context.last_selected_entity != entt::null && registry.valid(
                              m_selection_context.last_selected_entity);
            if (ImGui::MenuItem("Delete Selected", nullptr, false, hasSel)) {
                auto e = m_selection_context.last_selected_entity;
            ...
            if (ImGui::MenuItem("Focus Camera", nullptr, false, hasSel)) {
                ...
                if (e != entt::null && registry.valid(e)) {
                    ...
                    glm::vec3 center{0};
                    float radius = 1.0f;
                    if (registry.all_of<BoundingVolumeAABBComponent>(e)) {
                        const auto &bv = registry.get<BoundingVolumeAABBComponent>(e);
                    ...
                    entt::entity cam = CameraUtils::GetCameraEntityPrimary(registry);
                    if (cam != entt::null && registry.valid(cam) && registry.all_of<TransformLocal>(cam)) {
                        auto &tl = registry.get<TransformLocal>(cam);
                        ...
                        CameraUtils::SetCameraDirty(registry, cam);
                    }
                }
            }
            if (ImGui::MenuItem("Deselect", nullptr, false, m_selection_context.last_selected_entity != entt::null)) {
                m_selection_context.last_selected_entity = entt::null;
            }
            ImGui::EndMenu();
        }
        ...
        entt::entity selected = m_selection_context.last_selected_entity;
    ...
    std::string EditorLayer::get_entity_label(entt::entity e, bool selected) const {
        const auto &registry = m_scene.get_registry();
        std::string base;
        if (registry.valid(e)) {
            if (registry.all_of<NameTagComponent>(e)) {
                base = registry.get<NameTagComponent>(e).name;
            } else {
    ...
    void EditorLayer::draw_entity_list() {
        ...
        auto view = registry.view<entt::entity>(); // iteration over alive entities
        int count = 0;
        for (auto entity: view) {
            ++count;
            std::string label = get_entity_label(entity, entity == m_selection_context.last_selected_entity);
            if (!filter.empty()) {
            ...
            bool isSelected = entity == m_selection_context.last_selected_entity;
            if (ImGui::Selectable(label.c_str(), isSelected)) {
                m_selection_context.last_selected_entity = entity;
            }
            ImGui::PopID();
        }
    ...
    bool EditorLayer::pick_at_cursor(entt::entity &out_entity) const {
        ...
        entt::entity cam = CameraUtils::GetCameraEntityPrimary(registry);
        if (cam == entt::null || !registry.valid(cam)) return false;
        if (!registry.all_of<CameraMatrices>(cam)) return false;
        const auto &cm = registry.get<CameraMatrices>(cam);
        glm::vec3 camPos{0};
        if (registry.all_of<TransformWorld>(cam)) camPos = glm::vec3(registry.get<TransformWorld>(cam).matrix[3]);
        else if (registry.all_of<TransformLocal>(cam)) camPos = registry.get<TransformLocal>(cam).translation;
        ...
        auto view = m_scene.get_registry().view<BoundingVolumeAABBComponent>();
        for (auto e: view) {
