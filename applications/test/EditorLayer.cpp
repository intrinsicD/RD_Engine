...
namespace RDE {
    ...
    static bool ray_intersect_aabb(const glm::vec3 &ray_o, const glm::vec3 &ray_d, const AABB &aabb, float &tmin_out) {
        // Slabs method
        float tmin = -std::numeric_limits<float>::infinity();
        float tmax =  std::numeric_limits<float>::infinity();
        for(int i=0;i<3;++i){
            float invD = 1.0f / (ray_d[i] == 0.0f ? 1e-20f : ray_d[i]);
            float t0 = (aabb.min[i] - ray_o[i]) * invD;
            float t1 = (aabb.max[i] - ray_o[i]) * invD;
        ...
        tmin_out = tmin;
        return tmax >= 0.0f; // intersection in front
    }
    ...
    void EditorLayer::on_render_gui() {
        ...
        if (ImGui::BeginMenu("Editor")) {
            if(ImGui::MenuItem("Create Empty")) {
                ...
                if(!m_registry.all_of<NameTagComponent>(e)) m_registry.emplace<NameTagComponent>(e, NameTagComponent{"Entity"});
                if(m_app) m_app->set_last_selected_entity(e);
            }
            bool hasSel = m_app && m_app->get_last_selected_entity()!=entt::null && m_registry.valid(m_app->get_last_selected_entity());
            if(ImGui::MenuItem("Delete Selected", nullptr, false, hasSel)) {
                auto e = m_app->get_last_selected_entity();
                if(e!=entt::null && m_registry.valid(e)) { m_registry.destroy(e); m_app->set_last_selected_entity(entt::null); }
            }
            if(ImGui::MenuItem("Focus Camera", nullptr, false, hasSel)) {
                auto e = m_app->get_last_selected_entity();
                if(e!=entt::null && m_registry.valid(e)) {
                    // Compute center/radius
                    glm::vec3 center{0}; float radius = 1.0f;
    ...
    bool EditorLayer::pick_at_cursor(entt::entity &out_entity) const {
        ...
        if(!win) return false;
        int w=0,h=0; win->get_framebuffer_size(w,h);
        ...
        glm::vec4 ray_eye = inv_proj * ray_clip; ray_eye = glm::vec4(ray_eye.x, ray_eye.y, -1.0f, 0.0f);
        ...
        for(auto e : view) {
            ...
            float t; if(ray_intersect_aabb(camPos, ray_dir, bv.world, t)) {
                if(t >= 0.0f && t < best_t) { best_t = t; best = e; }
            }
        }
        if(best != entt::null) { out_entity = best; return true; }
        return false;
    }
}
